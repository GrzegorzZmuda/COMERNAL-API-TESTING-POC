...
def test_GET_sensordata_list_by_city_response_code(id=2):
    response = requests.get(BASE + "/city/" + str(id) + "/sensor/sensordata", headers=correct_header_1)
    assert response.status_code == 200
@pytest.mark.run(order=1)
def test_GET_sensordata_list_by_city_response_content(id=2):
    response = requests.get(BASE + "/city/" + str(id) + "/sensor/sensordata", headers=correct_header_1)
    assert response.headers.get('content-type') == 'application/json'
@pytest.mark.run(order=1)
def test_GET_sensordata_list_by_city_response_code_no_auth(id=2):
    response = requests.get(BASE + "/city/" + str(id) + "/sensor/sensordata")
    assert response.status_code == 401
@pytest.mark.run(order=1)
def test_GET_sensordata_list_by_city_response_code_unauth(id=2):
    response = requests.get(BASE + "/city/" + str(id) + "/sensor/sensordata", headers=incorrect_header_1)
    assert response.status_code == 401

@pytest.mark.run(order=1)
def test_GET_sensordata_list_by_city_response_code_not_found(id=999999):
    response = requests.get(BASE + "/city/" + str(id) + "/sensor/sensordata", headers=correct_header_1)
    assert response.status_code == 404

@pytest.mark.run(order=1)
def test_GET_sensordata_list_by_city_response_code_bad_value(id="k55543e"):
    response = requests.get(BASE + "/city/" + str(id) + "/sensor/sensordata", headers=correct_header_1)
    assert response.status_code == 404

@pytest.mark.run(order=1)
def test_GET_sensordata_list_by_city_response_types(id=2):
    response = requests.get(BASE + "/city/" + str(id) + "/sensor/sensordata", headers=correct_header_1)
...
@pytest.mark.run(order=1)
def test_GET_sensordata_list_by_city_response_id(id=2):
    response = requests.get(BASE + "/city/" + str(id) + "/sensor/sensordata", headers=correct_header_1)
    response_content = json.loads(response.content.decode())
    for i in range(len(response_content)):
