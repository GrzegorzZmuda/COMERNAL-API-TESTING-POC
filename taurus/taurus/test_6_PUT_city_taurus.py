...
def test_PUT_city_response_code(id=2):
    response = requests.put(BASE +"/city/"+str(id),gen_body_inhabitants() ,headers=correct_header_1)
    assert response.status_code==200
@pytest.mark.run(order=2)
def test_PUT_city_response_code_no_cityname(id=2):
    response = requests.put(BASE +"/city/"+str(id),gen_body_inhabitants() ,headers=correct_header_1)
    assert response.status_code==200
@pytest.mark.run(order=2)
def test_PUT_city_response_code_no_inhabitants(id=2):
    response = requests.put(BASE +"/city/"+str(id),gen_body_cityname() ,headers=correct_header_1)
    assert response.status_code==200
@pytest.mark.run(order=2)
def test_PUT_city_response_code_empty_body(id=2):
    response = requests.put(BASE +"/city/"+str(id),gen_body_empty() ,headers=correct_header_1)
    assert response.status_code==200
@pytest.mark.run(order=2)
def test_PUT_city_response_code_str_as_inhabitants(id=2):
    response = requests.put(BASE + "/city/"+str(id), gen_body_full_str_as_inhabitants(), headers=correct_header_1)
    assert response.status_code == 400

@pytest.mark.run(order=2)
def test_PUT_city_response_content(id=2):
    response = requests.put(BASE +"/city/"+str(id),gen_body_full() ,headers=correct_header_1)
    assert response.headers.get('content-type') == 'application/json'
@pytest.mark.run(order=2)
def test_PUT_city_response_content_no_cityname(id=2):
    response = requests.put(BASE +"/city/"+str(id),gen_body_inhabitants() ,headers=correct_header_1)
    assert response.headers.get('content-type') == 'application/json'
@pytest.mark.run(order=2)
def test_PUT_city_response_content_no_inhabitants(id=2):
    response = requests.put(BASE +"/city/"+str(id),gen_body_cityname() ,headers=correct_header_1)
    assert response.headers.get('content-type') == 'application/json'
@pytest.mark.run(order=2)
def test_PUT_city_response_content_empty_body(id=2):
    response = requests.put(BASE +"/city/"+str(id),gen_body_empty() ,headers=correct_header_1)
    assert response.headers.get('content-type') == 'application/json'
@pytest.mark.run(order=2)
def test_PUT_city_response_content_str_as_inhabitants(id=2):
    response = requests.put(BASE +"/city/"+str(id),gen_body_full_str_as_inhabitants() ,headers=correct_header_1)
    assert response.headers.get('content-type') == 'application/json'

@pytest.mark.run(order=2)
def test_PUT_city_response_inside_content(id=2):
...
@pytest.mark.run(order=2)
def test_PUT_city_response_inside_content_no_cityname(id=2):
    ...
    assert type(response_content["id"]) == int
@pytest.mark.run(order=2)
def test_PUT_city_response_inside_content_no_inhabitants(id=2):
    ...
    assert type(response_content["id"]) == int
@pytest.mark.run(order=2)
def test_PUT_city_response_inside_content_empty_body(id=2):
    response = requests.put(BASE +"/city/"+str(id),gen_body_empty() ,headers=correct_header_1)
    response_content = json.loads(response.content.decode())
    assert type(response_content["id"]) == int
@pytest.mark.run(order=2)
def test_PUT_city_response_inside_content_str_as_inhabitants(id=2):
    response = requests.put(BASE +"/city/"+str(id),gen_body_full_str_as_inhabitants() ,headers=correct_header_1)
    response_content = json.loads(response.content.decode())
...
@pytest.mark.run(order=2)
def test_PUT_city_response_inside_content_correct_message_type_str_as_inhabitants(id=2):
    response = requests.put(BASE +"/city/"+str(id),gen_body_full_str_as_inhabitants() ,headers=correct_header_1)
    response_content = json.loads(response.content.decode())
...
@pytest.mark.run(order=2)
def test_PUT_city_response_inside_content_correct_message_str_as_inhabitants(id=2):
    response = requests.put(BASE +"/city/"+str(id),gen_body_full_str_as_inhabitants() ,headers=correct_header_1)
    response_content = json.loads(response.content.decode())
    print(response_content["message"])
...
@pytest.mark.run(order=2)
def test_PUT_city_response_inside_content_correct_message_type_empty_inhabitants_field(id=2):
...
@pytest.mark.run(order=2)
def test_PUT_city_response_code_empty_inhabitants_field(id=2):
    correct_header_1_upg = {'Authorization': correct_header_1['Authorization'], 'Content-Type': 'application/json'}
